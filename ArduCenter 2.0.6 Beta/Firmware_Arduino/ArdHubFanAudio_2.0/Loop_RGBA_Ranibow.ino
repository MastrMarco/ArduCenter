...
void RGB_RainBow() {
  ...
  if (TimerVirtuale[1] >= (ResetTimerVirtuale[5] + DelayVirtuale[5])) {
    ...
    for (int i = 0; i < NUM_LEDS_OUT_All; i++) {
      ...
      if ((i >= NUM_LEDS_ALL) and (i < NUM_LEDS_OUT_8_9)) Strip2.setPixelColor(i - NUM_LEDS_ALL, Strip2.gamma32(Strip2.ColorHSV(pixelHue)));
      if ((i >= NUM_LEDS_OUT_8_9) and (i < NUM_LEDS_OUT_All)) Strip3.setPixelColor(i - (NUM_LEDS_ALL + NUM_LEDS_OUT_9), Strip2.gamma32(Strip3.ColorHSV(pixelHue)));
