//*****************************************************************************************************************************//
//      Bug Fix                                     Ver: 2.04 Firmware data 10/05/23                                           //
//*****************************************************************************************************************************//
      ...
      if (Boot_SetUp == 200) {
        ...
        case 2:
          ...
          if ((LumLED[ModLED_Fan] != DatoRX.toInt()) and ((DatoRX.toInt() >= 0) and (DatoRX.toInt() <= 255))) {
            ...
            // if ((DatoRX.toInt() < (LumLED[ModLED_Fan] - LumLimitLED)) and (PowerLimitLED == true or LumLED[ModLED_Fan] == 13)) {
            //   PowerLimitLED_Stato = false;
            // }

            //if (PowerLimitLED_Stato == false) LumLED[ModLED_Fan] = DatoRX.toInt();
            LumLED[ModLED_Fan] = DatoRX.toInt();
            //if ((PowerLimitLED_Stato == true) and (DatoRX.toInt() < (LumLED[ModLED_Fan] - LumLimitLED))) LumLED[ModLED_Fan] = DatoRX.toInt();

            if (ROM_Dati == true and PowerLimitLED_Stato == false) EEPROM.put(20, LumLED);  //Memorizza il valore nella cella 10 della EEPROM
          }

          //if (((ColoreLED[ModLED_Fan] > 512) and (BRIGHTNESS != (LumLED[ModLED_Fan] - LumLimitLED))) or ((ColoreLED[ModLED_Fan] <= 512) and (BRIGHTNESS != 255))) {
